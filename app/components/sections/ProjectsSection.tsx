...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({
  ...
  const { site, projects } = useWebBuilder();
  ...
  const display = useMemo<DisplayItem[]>(() => {
    ...
    const fromApi = (projects ?? []).filter((p) =>
      projectsSection.projectIds?.length
        ? projectsSection.projectIds.includes(p._id)
        : p.status === 'published'
    );
    return manual.length > 0 ? manual : fromApi;
  ...
  if (display.length === 0 && !projectsSection.title && !projectsSection.description) return null;
